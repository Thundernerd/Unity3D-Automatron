...
namespace TNRD.Automatron.Editor.Core {
    ...
    public class ExtendedNotification : ExtendedControl {
        ...
        public ExtendedNotification( string text, bool isError ) {
            ...
            Duration = 1.25f;
            AnchorPoint = TNRD.Automatron.Editor.EAnchor.TopLeft;
        }
        ...
        protected override void OnUpdate() {
            if ( !slideIn ) {
                Position.x = Mathf.MoveTowards( Position.x, Window.Size.x - ( Size.x + 25 ), ExtendedEditor.DeltaTime * outSpeed );
                if ( Position.x <= Window.Size.x - ( Size.x + 25 ) ) {
                    slideIn = true;
                }
            } else {
                if ( Duration > 0 ) {
                    Duration -= ExtendedEditor.DeltaTime;
                } else {
                    movingOut = true;
                    Position.y = Mathf.MoveTowards( Position.y, -( Size.y + 25 ), ExtendedEditor.DeltaTime * speed );
            ...
            if ( !movingOut ) {
                ...
                Position.y = Mathf.MoveTowards( Position.y, y, ExtendedEditor.DeltaTime * speed );
            }
        }
